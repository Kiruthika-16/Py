...
def update_data():
    ...
    mycursor.execute(query,(nameEntry.get(),phoneEntry.get(),emailEntry.get(),addressEntry.get(),genderEntry.get(),dobEntry.get()
...
def connect_database():
    def connect():
        ...
        try:
            ...
            mycursor=con.cursor()
            messagebox.showinfo('Success','Datebase Connection is Sucessful',parent=connectwindow)
        except:
...
exportstudentButton=ttk.Button(leftFrame,text='Export Student',width=15,state=DISABLED,command=export_data)
exportstudentButton.grid(row=6,column=0,pady=10)
exitButton=ttk.Button(leftFrame,text='Exit',width=15,command=iexit)
exitButton.grid(row=7,column=0,pady=18)

rightFrame=Frame(root)
rightFrame.place(x=350,y=80,width=820,height=480)
...
studentTable.heading('Address',text='Adress')
